...
def _prefilter_candidates(daily_df: pd.DataFrame, max_candidates: int = 80) -> pd.DataFrame:
    """
    ...
    # 기관 OR 외인 순매수 > 0
    supply_ok = (inst > 0) | (frgn > 0)
    # 등락률 온건
    change_ok = (chg >= -5) & (chg <= 5)
    # 거래대금 최소 (10억 미만 제외)
    vol_ok = vol_amt >= 1_000_000_000
...
def _check_rsi_recovery(df: pd.DataFrame, lookback: int = 10) -> tuple:
    """
    최근 lookback 거래일 내 RSI ≤ 30 (과매도)이었다가 현재 35~50 회복 여부.
    ...
    is_recovery = (min_rsi <= 32) and (35 <= current_rsi <= 50)
...
def _check_bollinger_bounce(df: pd.DataFrame) -> tuple:
    ...
    recent_pctb = df["BB_%B"].iloc[-5:-1]

    if pd.isna(current_pctb):
        return False, 0.0

    # 최근 5일 내 %B ≤ 0.1 터치 후 현재 0.1~0.5 (하단→중간 이동중)
    touched_lower = (recent_pctb <= 0.15).any()
    moving_up = 0.05 <= current_pctb <= 0.55
...
def _check_alignment_transition(df: pd.DataFrame) -> tuple:
    ...
    # 정배열초기 이거나 골든크로스 발생
    is_transition = (alignment in ("정배열초기",)) or gc
...
def _check_abcd_c_point(df: pd.DataFrame) -> bool:
    ...
    # 5~18% 조정 구간
    if not (-18 <= drawdown <= -3):
        return False

    # MACD 히스토그램 음→양 전환 (최근 3일 이내)
